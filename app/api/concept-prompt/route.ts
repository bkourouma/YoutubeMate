...
export async function POST(request: Request) {
  ...
  const system = `You are a senior YouTube thumbnail art director. Return only valid JSON matching {"prompt":"..."}. Write ONE detailed English image-generation prompt for a 16:9 YouTube thumbnail: one clear focal subject, precise composition, cinematic lighting, high contrast, and generous negative space. The prompt must contain no logo and no watermark. The headline is rendered into the image, so reserve a large uncluttered high-contrast area to hold it. The prompt may also call for a few short supporting text elements — a badge, a sticker, a number — and every word meant to appear in the image must be quoted exactly and written in ${language}. Ground the scene strictly in the supplied video concept; never invent brand names, real people, statistics, or claims. Keep the prompt between 40 and 120 words.`;
  const user = `TOPIC: ${safe(body.topic, 300)}\nVIDEO TITLE: ${title}\nTHUMBNAIL HEADLINE (rendered into the image, leave room for it, do not restate it in the prompt): ${safe(body.overlay, 120)}\nCONCEPT NAME: ${safe(body.conceptName, 160)}\nCHANNEL: ${safe(body.profile?.channel, 200)} — ${safe(body.profile?.theme, 500)}\nTHUMBNAIL EDITORIAL SYSTEM: ${safe(body.profile?.thumbnailSystemPrompt, 4_000) || "Not defined"}\nCURRENT PROMPT:\n${currentPrompt || "None yet"}\n\nTASK: ${direction ? `Rewrite the current prompt applying this direction (it may be written in ${language}): ${direction}` : "Rewrite the current prompt into a stronger, more specific variant of the same concept."}`;
