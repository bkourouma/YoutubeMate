...
export async function POST(request: Request) {
  ...
  if (cached?.results?.length === items.length) {
    await recordUsage({ userId, ...projectRef(body as Record<string, unknown>), pipeline: "shorts", action: "shorts-metadata", provider: "openrouter", usage: { ...cachedUsage(model) as TokenUsage, cost: 0, cacheHit: true } });
  ...
  try {
    ...
    await recordUsage({ userId, ...projectRef(body as Record<string, unknown>), pipeline: "shorts", action: "shorts-metadata", provider: "openrouter", usage: total });
