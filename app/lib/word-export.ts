...
export async function buildWordDocument(input: WordExportInput) {
  ...
  children.push(new Paragraph({
    spacing: { after: 40 },
    children: [new TextRun({ text: input.company || "YoutubeMate", bold: true, size: 30, color: ACCENT, characterSpacing: 16 })],
  }));
  children.push(new Paragraph({
    spacing: { after: 320 },
  ...
  const document = new Document({
    creator: input.company || "YoutubeMate",
    title: input.title,
    description: t(lang, "Document de travail généré par YoutubeMate", "Working document generated by YoutubeMate"),
    styles: {
      default: {
      ...
      headers: {
        default: new Header({
          children: [new Paragraph({
            ...
            children: [new TextRun({ text: `${input.company || "YoutubeMate"} · ${input.title}`, size: 17, color: MUTED })],
          })],
        }),
      },
      footers: {
